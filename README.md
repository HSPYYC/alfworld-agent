...
本次全量 unseen 正式结果：整体成功率 92/134 = 68.7%，平均环境动作数 17.01，非法动作率 32.2%，parse failure rate 5.2%。这里的 `--max-steps 50` 表示最多 50 次 ALFWorld `env.step`；`think:` 不消耗环境步。任务三提交版报告见 `docs/task3_submission_report.md`。
...
2. `alfworld-agent-learning`：学习全量版，保留完整工作区的学习版文档、探索记录、测试和额外材料。
